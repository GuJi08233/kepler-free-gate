...
使用 Go 实现的 Kepler AI 免费模型反代网关。网关从公共代理池选取可用代理，并支持按请求轮换 HTTP、HTTPS、SOCKS5/SOCKS5H 代理。
...
- 自动模式按“公共 S 级代理 → ZenProxy → 自定义代理 → 直连”回退。
...
```bash
docker build -t kepler-free-gate:local .
```
