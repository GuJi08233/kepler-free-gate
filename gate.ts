...
async function dispatchViaCustom(
  ...
  if (customSlots.length === 0 || retry >= maxRetries) {
    console.log(`[直连] 自定义代理耗尽(${retry}/${maxRetries})，直接连接上游`);
    return dispatchDirect(path, method, headers, body);
  }
  ...
  try {
    if (isStream) {
      return new Promise((resolve, reject) => {
        ...
        const firstByteTimer = setTimeout(() => req.destroy(new Error('直连超时')), PROXY_FIRST_BYTE_TIMEOUT);
        req.on('error', (e) => { clearTimeout(firstByteTimer); reject(e); });
        if (body) req.write(body);
        req.end();
      });
...
async function handleRequest(method: string, pathname: string, search: string, headers: Record<string, string>, body?: string, reqLog?: RequestLog): Promise<{ status: number; headers: Record<string, string>; body?: string; stream?: any }> {
  ...
  if (pathname === '/v1/chat/completions' && method === 'POST') {
    ...
    return Promise.race([
      dispatch('/v1/chat/completions', 'POST', h, body, 0, new Set<string>(), reqLog),
      new Promise<{ status: number; headers: Record<string, string>; body: string }>((_, reject) =>
        setTimeout(() => reject(new Error('请求超时')), HARD_TIMEOUT)
      ),
    ]).catch((e: Error) => {
      console.log(`[超时] POST /v1/chat/completions | ${e.message}`);
      return { status: 504, headers: { 'content-type': 'application/json' }, body: JSON.stringify({ error: e.message }) };
    });
  }
  ...
  globalThis.Bun.serve({
    ...
    async fetch(req: Request) {
      ...
      const result = await Promise.race([
        handleRequest(method, pathname, search, headers, body, reqLog),
        new Promise<{ status: number; headers: Record<string, string>; body?: string; stream?: any }>((_, reject) =>
          setTimeout(() => reject(new Error('请求超时')), HARD_TIMEOUT)
        ),
      ]).catch((e: Error) => {
        console.log(`[超时] ${method} ${pathname} | ${e.message}`);
